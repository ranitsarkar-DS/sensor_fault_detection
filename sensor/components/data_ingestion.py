...
from sensor.utils import export_colletion_as_dataframe
...
class DataIngestion:
    ...
    def initiate_data_ingestion(self)->DataIngestionArtifact:
        try:
            ...
            df = export_colletion_as_dataframe(database_name=self.data_ingestion_config.database_name,collection_name=self.data_ingestion_config.collection_name)

            logging.info("Replacing na with NAN")
            df.replace({"na":np.NAN},inplace=True)

            logging.info("Splittig dataframe into train and test")
